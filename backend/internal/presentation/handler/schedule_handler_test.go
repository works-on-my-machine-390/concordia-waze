package handler

import (
	"bytes"
	"context"
	"encoding/json"
	"net/http"
	"net/http/httptest"
	"testing"

	"github.com/gin-gonic/gin"
	"github.com/stretchr/testify/require"
	"github.com/works-on-my-machine-390/concordia-waze/internal/domain"
)

type fakeScheduleService struct {
	createClassFn     func(ctx context.Context, userID, title string) error
	getUserClassesFn  func(ctx context.Context, userID string) ([]string, error)
	getClassItemsFn   func(ctx context.Context, userID, title string) ([]domain.ClassItem, error)
	addClassItemFn    func(ctx context.Context, userID, title string, item domain.ClassItem) (string, error)
	updateClassItemFn func(ctx context.Context, userID, title, classID string, updates map[string]interface{}) error
	deleteClassItemFn func(ctx context.Context, userID, title, classID string) error
	deleteClassFn     func(ctx context.Context, userID, title string) error
}

func (f *fakeScheduleService) CreateClass(ctx context.Context, userID, title string) error {
	if f.createClassFn != nil {
		return f.createClassFn(ctx, userID, title)
	}
	return nil
}

func (f *fakeScheduleService) GetUserClasses(ctx context.Context, userID string) ([]string, error) {
	if f.getUserClassesFn != nil {
		return f.getUserClassesFn(ctx, userID)
	}
	return []string{}, nil
}

func (f *fakeScheduleService) GetClassItems(ctx context.Context, userID, title string) ([]domain.ClassItem, error) {
	if f.getClassItemsFn != nil {
		return f.getClassItemsFn(ctx, userID, title)
	}
	return []domain.ClassItem{}, nil
}

func (f *fakeScheduleService) AddClassItem(ctx context.Context, userID, title string, item domain.ClassItem) (string, error) {
	if f.addClassItemFn != nil {
		return f.addClassItemFn(ctx, userID, title, item)
	}
	return "class_1", nil
}

func (f *fakeScheduleService) UpdateClassItem(ctx context.Context, userID, title, classID string, updates map[string]interface{}) error {
	if f.updateClassItemFn != nil {
		return f.updateClassItemFn(ctx, userID, title, classID, updates)
	}
	return nil
}

func (f *fakeScheduleService) DeleteClassItem(ctx context.Context, userID, title, classID string) error {
	if f.deleteClassItemFn != nil {
		return f.deleteClassItemFn(ctx, userID, title, classID)
	}
	return nil
}

func (f *fakeScheduleService) DeleteClass(ctx context.Context, userID, title string) error {
	if f.deleteClassFn != nil {
		return f.deleteClassFn(ctx, userID, title)
	}
	return nil
}

func TestCreateCourseSuccess(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		createClassFn: func(ctx context.Context, userID, title string) error {
			require.Equal(t, "u1", userID)
			require.Equal(t, "SOEN 384", title)
			return nil
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.POST("/users/:userId/courses", h.CreateCourse)

	body := []byte(`{"name":"SOEN 384"}`)
	req := httptest.NewRequest(http.MethodPost, "/users/u1/courses", bytes.NewBuffer(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusCreated, w.Code)
	require.Contains(t, w.Body.String(), "course created")
}

func TestCreateCourseBadRequest(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := NewScheduleHandler(&fakeScheduleService{})

	r := gin.New()
	r.POST("/users/:userId/courses", h.CreateCourse)

	req := httptest.NewRequest(http.MethodPost, "/users/u1/courses", bytes.NewBufferString(`{"name":}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusBadRequest, w.Code)
}

func TestGetCoursesSuccess(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		getUserClassesFn: func(ctx context.Context, userID string) ([]string, error) {
			require.Equal(t, "u1", userID)
			return []string{"SOEN 384"}, nil
		},
		getClassItemsFn: func(ctx context.Context, userID, title string) ([]domain.ClassItem, error) {
			require.Equal(t, "u1", userID)
			require.Equal(t, "SOEN 384", title)
			return []domain.ClassItem{{
				ClassID:   "c1",
				Type:      "Lecture",
				Section:   "N",
				Day:       "MON",
				StartTime: "10:00",
				EndTime:   "12:00",
			}}, nil
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.GET("/users/:userId/courses", h.GetCourses)

	req := httptest.NewRequest(http.MethodGet, "/users/u1/courses", nil)
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusOK, w.Code)

	var courses []domain.CourseItem
	err := json.Unmarshal(w.Body.Bytes(), &courses)
	require.NoError(t, err)
	require.Len(t, courses, 1)
	require.Equal(t, "SOEN 384", courses[0].Name)
	require.Len(t, courses[0].Classes, 1)
	require.Equal(t, "c1", courses[0].Classes[0].ClassID)
}

func TestDeleteCourseSuccess(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		deleteClassFn: func(ctx context.Context, userID, title string) error {
			require.Equal(t, "u1", userID)
			require.Equal(t, "SOEN 384", title)
			return nil
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.DELETE("/users/:userId/courses/:title", h.DeleteCourse)

	req := httptest.NewRequest(http.MethodDelete, "/users/u1/courses/SOEN%20384", nil)
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusOK, w.Code)
}

func TestDeleteCourseNotFound(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		deleteClassFn: func(ctx context.Context, userID, title string) error {
			return domain.ErrCourseNotFound
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.DELETE("/users/:userId/courses/:title", h.DeleteCourse)

	req := httptest.NewRequest(http.MethodDelete, "/users/u1/courses/SOEN%20384", nil)
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusNotFound, w.Code)
}

func TestAddClassItemSuccess(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		addClassItemFn: func(ctx context.Context, userID, title string, item domain.ClassItem) (string, error) {
			require.Equal(t, "u1", userID)
			require.Equal(t, "SOEN 384", title)
			require.Equal(t, "Lecture", item.Type)
			return "class_123", nil
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.POST("/users/:userId/courses/:title/classes", h.AddClassItem)

	body := []byte(`{"type":"Lecture","section":"N","day":"MON","startTime":"10:00","endTime":"12:00","buildingCode":"H","room":"110","origin":"manual"}`)
	req := httptest.NewRequest(http.MethodPost, "/users/u1/courses/SOEN%20384/classes", bytes.NewBuffer(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusCreated, w.Code)
	require.Contains(t, w.Body.String(), "class_123")
}

func TestAddClassItemNotFound(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		addClassItemFn: func(ctx context.Context, userID, title string, item domain.ClassItem) (string, error) {
			return "", domain.ErrCourseNotFound
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.POST("/users/:userId/courses/:title/classes", h.AddClassItem)

	body := []byte(`{"type":"Lecture","section":"N","day":"MON","startTime":"10:00","endTime":"12:00"}`)
	req := httptest.NewRequest(http.MethodPost, "/users/u1/courses/SOEN%20384/classes", bytes.NewBuffer(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusNotFound, w.Code)
}

func TestUpdateClassItemSuccess(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		updateClassItemFn: func(ctx context.Context, userID, title, classID string, updates map[string]interface{}) error {
			require.Equal(t, "u1", userID)
			require.Equal(t, "SOEN 384", title)
			require.Equal(t, "class_123", classID)
			require.Equal(t, "115", updates["room"])
			return nil
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.PUT("/users/:userId/courses/:title/classes/:classId", h.UpdateClassItem)

	body := []byte(`{"room":"115","endTime":"12:15"}`)
	req := httptest.NewRequest(http.MethodPut, "/users/u1/courses/SOEN%20384/classes/class_123", bytes.NewBuffer(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusOK, w.Code)
}

func TestUpdateClassItemBadRequest(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := NewScheduleHandler(&fakeScheduleService{})

	r := gin.New()
	r.PUT("/users/:userId/courses/:title/classes/:classId", h.UpdateClassItem)

	req := httptest.NewRequest(http.MethodPut, "/users/u1/courses/SOEN%20384/classes/class_123", bytes.NewBufferString(`{"room":}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusBadRequest, w.Code)
}

func TestDeleteClassItemSuccess(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		deleteClassItemFn: func(ctx context.Context, userID, title, classID string) error {
			require.Equal(t, "u1", userID)
			require.Equal(t, "SOEN 384", title)
			require.Equal(t, "class_123", classID)
			return nil
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.DELETE("/users/:userId/courses/:title/classes/:classId", h.DeleteClassItem)

	req := httptest.NewRequest(http.MethodDelete, "/users/u1/courses/SOEN%20384/classes/class_123", nil)
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusOK, w.Code)
}

func TestDeleteClassItemNotFound(t *testing.T) {
	gin.SetMode(gin.TestMode)
	service := &fakeScheduleService{
		deleteClassItemFn: func(ctx context.Context, userID, title, classID string) error {
			return domain.ErrCourseNotFound
		},
	}
	h := NewScheduleHandler(service)

	r := gin.New()
	r.DELETE("/users/:userId/courses/:title/classes/:classId", h.DeleteClassItem)

	req := httptest.NewRequest(http.MethodDelete, "/users/u1/courses/SOEN%20384/classes/class_123", nil)
	w := httptest.NewRecorder()

	r.ServeHTTP(w, req)
	require.Equal(t, http.StatusNotFound, w.Code)
}
